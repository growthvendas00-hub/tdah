...
export function HabitsPage({ state, actions, openAccount, notify }: { state: TeamState; actions: TeamActions; openAccount: () => void; notify: (text: string) => void }) {
  ...
  return <div className="page inner-page habits-page">
    ...
    <div className="habit-grid">{mine.map((plan) => <HabitCard key={plan.id} plan={plan} state={state} actions={actions} notify={notify} />)}{(['tabaco', 'cannabis'] as HabitKind[]).filter((kind) => !mine.some((item) => item.kind === kind)).map((kind) => <button className="new-habit-card" key={kind} onClick={() => actions.createHabit(kind)}><Plus size={22} /><strong>Acompanhar {copy[kind].title.toLowerCase()}</strong><span>Começar com uma referência editável</span></button>)}</div>
...
function HabitCard({ plan, state, actions, notify }: { plan: HabitPlan; state: TeamState; actions: TeamActions; notify: (text: string) => void }) {
  ...
  const recent = Array.from({ length: 7 }, (_, index) => dayOffset(index - 6)).map((date) => ({ date, amount: state.habitLogs.find((item) => item.planId === plan.id && item.date === date)?.amount }))
  const max = Math.max(plan.baseline, ...recent.map((item) => item.amount ?? 0), 1)
  async function save() { await actions.logHabit(plan.id, amount); notify('Registro salvo. Honestidade vale mais que perfeição.') }
  return <article className="habit-card" style={{ '--habit': details.color } as React.CSSProperties}>
    <header><span><Icon size={21} /></span><div><small>{plan.mode === 'parar' ? 'CAMINHO PARA PARAR' : 'REDUÇÃO GRADUAL'}</small><h2>{details.title}</h2></div><button className={plan.shareWithWorkspace ? 'shared' : ''} onClick={() => actions.updateHabitPlan(plan.id, { shareWithWorkspace: !plan.shareWithWorkspace })}>{plan.shareWithWorkspace ? <Users size={15} /> : <Lock size={15} />}{plan.shareWithWorkspace ? 'Compartilhado' : 'Só eu'}</button></header>
    ...
    <div className="habit-bars">{recent.map((item) => <div key={item.date}><span>{item.amount ?? '–'}</span><i><b style={{ height: `${item.amount === undefined ? 4 : Math.max(8, item.amount / max * 100)}%` }} /></i><small>{new Intl.DateTimeFormat('pt-BR', { weekday: 'short' }).format(new Date(`${item.date}T12:00:00`)).replace('.', '')}</small></div>)}</div>
    <div className="habit-log"><div><button onClick={() => setAmount(Math.max(0, amount - 1))} aria-label="Diminuir"><Minus size={16} /></button><strong>{amount}</strong><button onClick={() => setAmount(amount + 1)} aria-label="Aumentar"><Plus size={16} /></button><span>{plan.unit}</span></div><button className="primary-button" onClick={save}><Check size={15} />Registrar hoje</button></div>
    <button className="habit-edit-toggle" onClick={() => setEditing(!editing)}>Editar referência e meta <ChevronRight size={14} /></button>
    {editing && <div className="habit-edit"><label>Referência<input type="number" min="0" value={plan.baseline} onChange={(event) => actions.updateHabitPlan(plan.id, { baseline: Number(event.target.value) })} /></label><label>Alvo<input type="number" min="0" value={plan.target} onChange={(event) => actions.updateHabitPlan(plan.id, { target: Number(event.target.value) })} /></label><label>Estratégia<select value={plan.mode} onChange={(event) => actions.updateHabitPlan(plan.id, { mode: event.target.value as HabitPlan['mode'] })}><option value="reduzir">Reduzir</option><option value="parar">Parar</option></select></label></div>}
    <p className="habit-message">{todayLog ? 'Você registrou hoje. Se o número mudar, pode atualizar.' : amount <= plan.target ? 'Dentro do alvo escolhido. Reconheça esse passo.' : 'Acima do alvo não significa fracasso. Registre e observe o contexto.'}</p>
  </article>
}
