import { Bell, DatabaseBackup, Download, MonitorSmartphone, Moon, Palette, PlayCircle, RefreshCcw, RotateCcw, Save, ShieldCheck, Sun, Upload, UserRound, X } from 'lucide-react'
...
import type { AppState, ColorMode } from '../types'
...
export function SettingsPage({ state, actions, teamState, teamActions, notify }: { state: AppState; actions: FocoActions; teamState: TeamState; teamActions: TeamActions; notify: (text: string) => void }) {
  ...
  return <div className="page inner-page settings-page"><section className="page-heading"><div><p className="eyebrow">DO SEU JEITO</p><h1>Configurações</h1><p>Ajuste o espaço para reduzir atrito, estímulos e decisões desnecessárias.</p></div>{demoActive && <span className="demo-mode-badge"><PlayCircle size={15} />Demonstração ativa</span>}</section><div className="settings-layout">
    ...
    <section className="surface settings-section"><div className="settings-title"><span><Palette size={19} /></span><div><h2>Aparência</h2><p>Roxo energético com contraste confortável para cada ambiente.</p></div></div><div className="color-mode-options"><ColorModeButton mode="light" active={state.settings.colorMode === 'light'} onClick={() => actions.updateSettings({ colorMode: 'light' })} /><ColorModeButton mode="dark" active={state.settings.colorMode === 'dark'} onClick={() => actions.updateSettings({ colorMode: 'dark' })} /></div><Toggle icon={<Moon />} label="Reduzir movimentos" description="Diminui transições e celebrações visuais." checked={state.settings.reducedMotion} onChange={(value) => actions.updateSettings({ reducedMotion: value })} /></section>
...
function ColorModeButton({ mode, active, onClick }: { mode: ColorMode; active: boolean; onClick: () => void }) { const Icon = mode === 'light' ? Sun : Moon; return <button type="button" className={`${mode} ${active ? 'active' : ''}`} aria-pressed={active} onClick={onClick}><span><Icon size={19} /></span><div><strong>{mode === 'light' ? 'Modo claro' : 'Modo escuro'}</strong><small>{active ? 'Em uso' : 'Usar agora'}</small></div></button> }
